...
<head>
    ...
    <title>Requisitos Postulantes</title>
...
<body class="sb-nav-fixed">
    <nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark">
        <a class="navbar-brand" href="dashboardRequisitosPostulantes.php">Votaciones Uniminuto</a>
    ...
    <div id="layoutSidenav_content">
        <main>
            <div class="container-fluid">
                <h1 class="mt-4">Requisitos Postulantes</h1>
                <ol class="breadcrumb mb-4">
                    <li class="breadcrumb-item active">Requisitos Postulantes</li>
                </ol>
                      ...
                      <div class="row">
                        <div class="col-lg-3 col-xl-3 col-md-3">
                          <div class="form-group">
                                        ...
                                        <input  class="form-control py-4"type="text" name="codigo" id="codigo">
                          </div>
                        </div>
                        <div class="col-lg-3 col-xl-3 col-md-3">
                              <div class="form-group">
...
<script type="text/javascript">
    ...
    function getInfoRequisitosAll(){
      
      $.ajax({
        type: "get",
        url: '../../eleccionesBackEnd/consume/getInfoRequisitosAll.php',
        ...
        success: function (msg) 
        {
           ...
           $.each(r, function(item){
            ...
            $("#contentTable").append('<tr><td><a class="ancla" data-ancla="agregar" style="cursor:pointer;color:blue;text-decoration:underline" onclick="updateAdmin('+r[item].id+')">'+r[item].codigo+'</a></td><td>'+r[item].rol+'</td><td>'+r[item].descripcion+'</td><td>'+r[item].fecha+'</td><td>'+r[item].status+'</td></tr>');
           });
...
function recargarTabla(){
    $("#datosPOS").empty();
                $("#datosPOS").append('<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0"><thead><tr><th>Código</th><th>Rol</th><th>Descripción</th><th>Fecha</th><th>Status</th></tr></thead><tfoot><tr><th>Código</th><th>Rol</th><th>Descripción</th><th>Fecha</th><th>Status</th></tr>                            </tfoot><tbody id="contentTable"></tbody></table>');
                getInfoRequisitosAll();
}
function updateAdmin(id){
    ...
    //se hace el llamado ajax
        $.ajax({
                url: '../../eleccionesBackEnd/consume/getInfoRequisitosAllById.php?id='+id,
                type: 'get',
                ...
                success: function(response) {
                  ...
                  $.each(r, function(item){
                    $("#codigo").val(r[item].codigo);
                    $("#rol").val(r[item].rol);
                    $("#descripcion").val(r[item].descripcion);
                    $("#apellido2").val(r[item].apellido2);
...
$(document).ready(function()
{
     ...
     $("#agregar").click(function(){
        var url='';var confirma=''; var labelAgregar='';var id='';
        if($("#agregar").text()=='Actualizar'){
             url='../../eleccionesBackEnd/consume/actualizaRequisito.php';
             confirma='¿Desea actualizar los datos de este requisito de postulación?';
            ...
            url='../../eleccionesBackEnd/consume/addRequisito.php';
            confirma='¿Desea agregar este requisito de postulación al sistema?';
            labelAgregar='Guardar Datos';
            status='';
            id='';
        }
        if(confirm(confirma)){
           if($("#codigo").val()=='' || $("#rol").val()=='0' || $("#descripcion").val()=='' || $("#status").val()=='0'){
              alert('los campos marcados con * son obligatorios.');
                    ...
                    url: url,
                    type: 'post',
                    data: {codigo:codigo,rol:rol,descripcion:descripcion,status:status,id:id},
                    dataType:'json',
                    beforeSend: function()
                    {
                        // setting a timeout
